...
int main()
{
    ...
    printf("printing linked list from head:\n");
    printfromstart(head);
    printf("\nprinting linked list from tail:\n");
    printfromend(tail);
    del_gpterm(&head, &tail, gpfactor, n);
    printf("\nprinting linked list from head after deletion:\n");
    printfromstart(head);
    printf("\nprinting linked list from tail after deletion:\n");
    printfromend(tail);
    printf("\n");
    tail = NULL;
}
...
void del_gpterm(node **head_pointer, node **tail_pointer, int factor, unsigned int n)
{
    node *flag = *head_pointer; //flag is dummy varialble of head(dummy variable)
    node *previous = NULL;
    int del_index = 1;
    int i = 1;
    for (i = 1; i <= n; i++)
